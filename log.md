...
## [2026-04-28] ingest | enriquecer M6 Mapeo con libro Thrun et al. (Pasada 3, FINAL)
- **Disparador**: Última de cuatro pasadas para enriquecer (no reescribir) el contenido del vault con `Raw/Libro/ProbabilisticRobotics.pdf`. Pasada 3 cubre el M6 — Mapeo — basada en cap 9 (Occupancy Grid Mapping) y cap 10 (SLAM con EKF).
- **Páginas enriquecidas (4)**:
  - **`Mapeo` (overview)**: callout con los 4 factores de dificultad del mapeo (§9.1: tamaño, ruido, ambigüedad perceptual, ciclos — framing fundamental que motiva los distintos enfoques).
  - **`Mapas de Ocupación`**: callout `> [!warning]` sobre por qué la factorización es necesaria (§9.2: $2^{10000}$ mapas posibles intractable, factorización pierde dependencias entre celdas pero es lo que hace al algoritmo viable) + callout sobre por qué importa un algoritmo con poses conocidas (§9.1: utilidad real es post-procesamiento de SLAM, da mapas con geometría volumétrica para path planning) + callout sobre Multi-Sensor Fusion conservativa con `max` (§9.2.1: distintos sensores ven distintos obstáculos; max es más seguro que Bayes para path planning).
  - **`Mapeo con Poses Conocidas`**: callout sobre aprender el modelo inverso con red neuronal (§9.3: alternativa a hardcodear, useful para sensores complejos) + callout `> [!warning]` sobre limitación de la factorización + MAP Occupancy Mapping (§9.4: hill-climbing global produce mapas más coherentes sin las contradicciones del Bayes celda-por-celda).
  - **`SLAM - Mapeo y Localización Simultánea`** (la página vault era muy corta, "puente"): callout sobre Online vs Full SLAM (§10.1: framing fundamental — el state vector full crece con $t$, por eso EKF no puede hacer full SLAM) + callout sobre el "milagro" del EKF-SLAM (§10.2.2: observar 1 landmark mejora todos via off-diagonal del $\Sigma$, explica loop closure y el costo $O(N^2)$) + sección sustancial nueva `## 3. Limitaciones de EKF-SLAM y técnicas prácticas` (§10.2–§10.3: complejidad cuadrática, only online, brittleness, provisional landmark list, evidence counter en log-odds, spatial arrangement, signatures, justificación didáctica de seguir cubriéndolo).
- **Páginas NO enriquecidas**:
  - `Mapas de Grilla`: la página tiene cobertura suficiente del concepto base (discretización rígida, celdas binarias). El libro toca el tema brevemente en §6.2 sin agregar info adicional.
  - `Features vs Mapas Volumétricos`: la dicotomía paramétrico/no-paramétrico ya está bien cubierta. El libro la trata implícitamente en §6.6 y §10.2.1 sin nuevos puntos sustantivos.
  - `Exploración`: el libro no tiene capítulo dedicado a exploración en este draft (sería cap 17 en versión final que no figura en este PDF).
- **Forma del enriquecimiento**: idéntica a las pasadas previas — callouts `> [!info]` / `> [!warning]` con cita explícita `(Thrun et al., §X.Y)` + 1 sección sustancial nueva en `SLAM` por la cantidad de contenido del cap 10. Frontmatter de las 4 páginas suma libro a `fuentes:` (excepto `Mapeo` overview que no tiene frontmatter por ser overview puro) y avanza `ultima_actualizacion`; bloques `## Fuentes` ganan entrada con páginas específicas.
- **Resultado**: 4 páginas con +8 callouts y +1 sección nueva sustancial. La cobertura del M6 con el libro queda completa.
- **Cierre del proyecto** (4 pasadas, 4 commits): el vault queda con `Raw/Libro/ProbabilisticRobotics.pdf` integrado como **fuente secundaria catalogada** en 20 páginas distribuidas entre M1, M4, M5, M6. Total agregado: 41 callouts (`> [!info]` / `> [!warning]` / `> [!quote]`) + 4 secciones sustanciales nuevas (Profundización en Filtro de Bayes, Profundización en Modelo de Movimiento, Augmented MCL, Limitaciones de EKF-SLAM). Cero modificación a la prosa existente — sólo intervenciones quirúrgicas que citan explícitamente al libro, manteniendo la voz original del vault. El estado de cualquier página enriquecida puede revisarse buscando `Thrun et al.` en su archivo.

## [2026-04-28] ingest | enriquecer M5 Filtros Bayesianos con libro Thrun et al. (Pasada 2)
- **Disparador**: Tercera de cuatro pasadas para enriquecer (no reescribir) el contenido del vault con `Raw/Libro/ProbabilisticRobotics.pdf`. Pasada 2 cubre el M5 — Filtros Bayesianos — basada en cap 3 (Gaussian Filters: KF + EKF), cap 4 (Nonparametric Filters: histogram + particle filter), y cap 8 (Grid + Monte Carlo Localization).
- **Páginas enriquecidas (4)**:
  - **`Filtros Discretos`**: callout con la conexión a HMM (§4.1.1: el filtro discreto es el forward pass de un HMM, importa porque trae la maquinaria Viterbi/Baum-Welch/forward-backward) + callout con resoluciones típicas y trucos computacionales (§8.2.2–§8.2.3: 15 cm + 5° indoor, pre-caching del sensor model, sub-sampling de haces, delayed motion updates, selective updating) + callout `> [!warning]` sobre inflar ruido en grillas gruesas (§8.2.2: sin esto el filtro queda congelado entre cells).
  - **`Filtro de Kalman`**: callout con los 3 supuestos lineal-gaussianos formales (§3.2.1: explicita el supuesto del belief inicial gaussiano que el vault implícitamente saltea) + callout sobre integración reduce incertidumbre (§3.2.3: la posterior tras update es más concentrada que cualquiera de los contribuyentes, magia del KF que falla bajo correlación) + callout sobre costo computacional con algoritmos modernos (§3.2.2: $O(d^{2.8})$ inversión, $O(k^{2.8})$ por iter, escala bien a estados grandes con mediciones chicas — caso típico SLAM).
  - **`EKF`**: callout `> [!warning]` sobre cuándo falla la linealización (§3.3.4: dos factores combinados — no-linealidad y magnitud de incertidumbre; heurística práctica: keep state uncertainty small; explica por qué EKF funciona para tracking pero no para global localization) + callout sobre Multi-Hypothesis EKF (MHEKF, §3.3.4: extensión natural con mixture of gaussians cuando la unimodalidad falla) + callout sobre alternativas a Taylor (§3.3.4: UKF y moments matching como técnicas superiores).
  - **`MCL - Filtro de Partículas`** (la más enriquecida): callout sobre por qué low-variance sampling supera al independiente (§4.2.4: cobertura sistemática + $O(M)$ + menor varianza del estimador) + callout `> [!warning]` sobre no resamplear cuando el estado es estático (§4.2.4: resampling sin updates destruye diversidad sin ganar info; usar varianza de pesos como criterio) + callout sobre resource-adaptive (§8.3.2: $M$ se ajusta online, ventaja única vs KF/discretos) + sección sustancial nueva `## 8. Augmented MCL: recuperarse de kidnapping` (§8.3.3: random particle injection con tracking $w_{slow}$ vs $w_{fast}$ + smoothing exponencial — contenido suficiente para sección entera, no callout).
- **Páginas NO enriquecidas**:
  - `Filtros Bayesianos` (overview): la página es muy concisa (overview puro de módulo, sin frontmatter ni Fuentes), agregar callouts de framing paramétrico vs no-paramétrico la inflaría sin valor proporcional.
  - `Muestreo por Importancia`: §4.2.2 del libro tiene la formulación rigurosa, pero la página ya cubre lo mismo (proposal $g$, target $f$, pesos $f/g$, convergencia $O(1/\sqrt{M})$, conexión MCL como caso particular).
  - `UKF`, `Puntos Sigma`, `Descomposición de Cholesky`, `Gaussiana Multivariada`: el libro no tiene capítulo dedicado a UKF (lo menciona como técnica alternativa al EKF en §3.3.4 — ya capturado en el callout del EKF). Los otros tres son herramientas matemáticas que el libro asume.
- **Forma del enriquecimiento**: idéntica a Pasadas 1a y 1b — callouts `> [!info]` / `> [!warning]` con cita explícita `(Thrun et al., §X.Y)` + una sección sustancial cuando el contenido lo justifica (Augmented MCL en este caso). Frontmatter de las 4 páginas suma libro a `fuentes:` y avanza `ultima_actualizacion`; bloques `## Fuentes` ganan entrada con páginas específicas.
- **Resultado**: 4 páginas con +10 callouts y +1 sección nueva sustancial. La cobertura del M5 con el libro queda cohesionada — cada filtro cita los tradeoffs prácticos, supuestos formales y limitaciones que el libro explicita y las slides resumen.
- **Próximo**: Pasada 3 (última) — Mapeo del M6 basada en caps 9 y 10 del libro: Mapas de Ocupación (incluyendo el binary Bayes filter en log-odds form), Mapeo con Poses Conocidas, SLAM con EKF.

## [2026-04-28] ingest | enriquecer M4 modelos con libro Thrun et al. (Pasada 1b)
- **Disparador**: Continuación del plan de cuatro pasadas para enriquecer (no reescribir) el contenido del vault con `Raw/Libro/ProbabilisticRobotics.pdf`. Pasada 1b cubre los **modelos probabilísticos** del M4: motion models (cap 5 del libro) y sensor models (cap 6).
- **Páginas enriquecidas (7)**:
  - **`Modelo de Movimiento` (hub)**: callout con Cinemática Probabilística (§5.2.2: el modelo es una densidad sobre poses futuras, "smudge" de probabilidad) + callout con tradeoff Velocidad vs Odometría (§5.4: una para estimación y otra para planning probabilístico) + sección `## 8. Profundización: sobreestimar incertidumbre` (§5.1: inflar $\alpha_i$ hace al filtro robusto a violaciones de Markov).
  - **`Modelo de Movimiento (Velocidad)`**: callout con drives admitidos (§5.3: differential, Ackerman, synchro — no Mecanum/legged) + callout con justificación rigurosa del tercer parámetro $\gamma$ (§5.3.3: degeneración 2D dentro de 3D si sólo se usan dos ruidos) + callout con asimetría sample-vs-prob (§5.3.2: sample sólo necesita simulación forward; prob requiere invertir el modelo — razón por la que partícula filtros son atractivos).
  - **`Modelo de Movimiento (Odometría)`**: callout sobre odometría como sensor tratado como control (§5.4: convención del libro para mantener el state space chico) + callout sobre convergencia entre modelos a alta frecuencia (§5.4: a 10 Hz interior la diferencia velocidad/odometría es despreciable).
  - **`Modelo de Sensor` (hub)**: callout sobre por qué probabilizar permite usar modelos crudos (§6.1: la incertidumbre se desplaza al modelo probabilístico, ventaja central sobre robótica clásica) + callout `> [!warning]` sobre realismo físico ≠ utilidad para el filtro (§6.7: smoothness y sobreconfianza importan tanto como el realismo; truco $\alpha < 1$ para atenuar verosimilitud).
  - **`Modelo de Sensor Basado en Haz`**: callout sobre EM como método específico para ajustar parámetros (§6.3.2: latent variables $c_i$ + E-step/M-step con soluciones cerradas en eqs 6.31–6.32) + callout sobre subsampling de haces y atenuación con $\alpha < 1$ (§6.3.4: dos remedios prácticos para la sobreconfianza por correlación entre haces vecinos).
  - **`Modelo de Campo de Verosimilitud`**: callout `> [!warning]` con la idea central "ad hoc pero funciona" (§6.4: no es un modelo generativo del sensor, es un nearest-neighbor en 2D, pero supera al beam model en práctica por suavidad y eficiencia) + callout `> [!warning]` con tres limitaciones concretas (§6.4.2: no modela dinámica, "ve a través de paredes" por nearest-neighbor sin ray casting, no maneja zonas no exploradas; extensión: agregar categoría "unknown").
  - **`Modelo de Detección de Landmarks`**: callout sobre features como reducción de dimensión (§6.6.1: inferencia órdenes de magnitud más eficiente en feature space) + callout sobre signature + correspondence problem (§6.6.2–§6.6.3: tercer componente $s$ del feature vector + variable de correspondencia $c_t^i$ + data association) + callout `> [!warning]` sobre tradeoff features vs raw — analogía visual de Thrun (§6.6.5: features comprimen con pérdida, raw measurements suelen ganar en hardware moderno).
- **Página NO enriquecida**: `Muestreo de Distribuciones`. El libro presenta los algoritmos `sample_normal` (suma de 12 uniformes via CLT) y `sample_triangular` (suma de 2 uniformes) en §5.3.2, pero la página ya los cubre con la misma profundidad. Agregar callouts sería ruido.
- **Forma del enriquecimiento**: idéntica a Pasada 1a — callouts `> [!info]` / `> [!warning]` con cita explícita `(Thrun et al., §X.Y)` en el título, una sección nueva `## Profundización` (sólo en el hub `Modelo de Movimiento` por la importancia del concepto de sobreestimar incertidumbre). Frontmatter de las 7 páginas suma `Raw/Libro/ProbabilisticRobotics.pdf` a `fuentes:` y bloque `## Fuentes` agrega entrada con páginas específicas.
- **Resultado**: 7 páginas con +14 callouts y +1 sección de profundización. La línea editorial del módulo M4 queda cohesionada con el libro: cada modelo cita los lugares específicos donde Thrun aporta intuición, justificación rigurosa, tradeoffs prácticos o limitaciones que las slides no documentan.
- **Próximo**: Pasada 2 — Filtros Bayesianos del M5 (cap 3 + cap 4 + cap 8 del libro): KF (intuición lineal-gaussiana sin derivación), EKF (practical considerations), Filtros Discretos, MCL (random particle, sample impoverishment), Muestreo por Importancia.

## [2026-04-28] ingest | enriquecer M4 fundamentos con libro Thrun et al. (Pasada 1a)
- **Disparador**: Valentino agregó `Raw/Libro/ProbabilisticRobotics.pdf` (Thrun, Burgard & Fox, *Probabilistic Robotics*, early draft 1999-2000, 492 págs) y pidió usarlo para *enriquecer* contenido existente, no para agregar nuevos temas. Estrategia acordada: cuatro pasadas (1a M4 fundamentos, 1b M4 modelos, 2 M5, 3 M6), forma del enriquecimiento mediante callouts `> [!info] (Thrun et al., §X)` y secciones `## Profundización` antes de `## Fuentes`, sin reescribir las páginas existentes. Libro queda agregado al frontmatter `fuentes:` y al bloque `## Fuentes` con páginas específicas de cada capítulo/sección.
- **Pasada 1a** — fundamentos probabilísticos del M4, basada en cap 1 + cap 2 del libro. 5 páginas tocadas:
  - **`Introducción a la Robótica Probabilística`**: callout con las 5 fuentes de incertidumbre (§1.1: entornos, sensores, actuadores, modelos, computación) + callout con la conjetura central (§1.3: *"un robot que conoce su propia incertidumbre y actúa en consecuencia es superior a uno que no"*) + sección `## Profundización: implicaciones` con tradeoffs computacionales y casos donde el enfoque probabilístico es la única solución conocida (kidnapped robot, mapeo sin GPS, aprendizaje robótico).
  - **`Filtro de Bayes`**: callout con notación predict/correct (§2.3.4: $\overline{bel}(x_t)$ vs $bel(x_t)$ como esqueleto común a KF, EKF, MCL) + sección `## Profundización: ¿por qué hay tantos filtros?` con los 3 tradeoffs de §2.5 (eficiencia computacional, exactitud, facilidad de implementación) que justifican la división del M5 en familias.
  - **`Derivación del Filtro de Bayes`**: callout con supuestos de validez (§2.4.3: estado completo + controles aleatorios) + callout con equivalencia HMM/DBN (§2.3.3) que conecta con técnicas adyacentes (Viterbi, Baum-Welch).
  - **`Suposición de Markov`**: callout `> [!warning]` con las 4 violaciones típicas (§2.4.4: dinámica no modelada, inexactitudes de modelo, errores de aproximación del belief, variables del software de control) + callout con la robustez práctica que justifica usar estados incompletos.
  - **`Regla de Bayes`**: callout sobre modelo generativo / inverso (§2.2: $P(z \mid x)$ es causal/fácil, Bayes lo invierte en diagnóstico) + callout `> [!warning]` sobre independencia condicional ≠ absoluta (§2.2: ninguna implica la otra; $z_1, z_2$ son condicionalmente independientes dado $x$ pero no en absoluto).
- **Páginas que NO se enriquecieron** (libro no aporta sobre las slides):
  - `Axiomas de Probabilidad`: el libro arranca con PMF discreta y no expone los axiomas de Kolmogorov; las slides son más explícitas en este punto.
  - `Probabilidad Conjunta`, `Probabilidad Condicional`, `Variables Aleatorias Discretas`, `Variables Aleatorias Continuas`: §2.2 del libro tiene definiciones equivalentes a las slides; agregar callouts sería ruido. El concepto de independencia condicional ≠ absoluta (única adición real de §2.2) se ubicó en `Regla de Bayes` que ya tiene una sección dedicada.
  - `Introducción a la Robótica Móvil` (M1): cap 1 del libro encaja mejor en `Introducción a la Robótica Probabilística`; duplicarlo sería redundante.
- **Forma de los enriquecimientos**: se respetó la voz y estructura existentes. Cada callout cita explícitamente sección del libro (§X.Y) en su título; cada bloque `## Fuentes` agregó una entrada `Raw/Libro/ProbabilisticRobotics.pdf` con páginas específicas; cada frontmatter sumó el libro a `fuentes:` y avanzó `ultima_actualizacion` a 2026-04-28.
- **Resultado**: 5 páginas con +9 callouts y +2 secciones de profundización, sin tocar prosa existente. El libro pasa de invisible a fuente secundaria catalogada en M4 fundamentos.
- **Próximo**: Pasada 1b — modelos probabilísticos del M4 (movimiento + sensor) basada en caps 5 y 6 del libro.
